=== i2Clipart ===
Contributors: sciweavers
Donate link: http://www.sciweavers.org/
Tags: clipart, clip art, graphics, vector images, pictures, images, public domain, royalty free
Requires at least: 2.7
Tested up to: 3.9.1
Stable tag: 4.3
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

TinyMCE plugin to search and insert royalty free public domain images from i2clipart.com to your page or post.

== Description ==

A TinyMCE plugin that allows you to insert free clipart images to posts on your Wordpress site. During writing a post, if you click the i2clipart plugin button, a popup dialog will show up containing a search box and a color palette. You can search for an image by typing the proper keywords or provide its dominant color through the color palette. The plugin will search i2clipart.com and display paginated search results in the form of small thumbnails. When you click a thumbnail, the associated image will be inserted into your TinyMCE editor. Every image comes in four different sizes (64px, 128px, 256px, 512px). Five buttons are displayed under every thumbnail. The first four buttons are labeled (S, M, L, X) describing image size in PNG format. The last button will show a set of thumbnail images having several color variations of the selected thumbnail. All provided images are 100% royalty free public domain clipart images.

== Installation ==

1. Download and unzip the plugin
2. Upload the entire "i2clipart/" directory to the "/wp-content/plugins/" directory
3. Activate the plugin through the "Plugins" menu in WordPress

== Frequently Asked Questions ==

= Can I use these images for commercial purposes? =
 Images on i2clipart.com are believed to be in the public domain (PD) i.e., you can use them for whatever purpose even commercial one. Images are collected from sources that place them in PD or are uploaded and released by our users in PD. Images are provided with absolutely no warranty. Use according to your country or jurisdiction

= Do I have to link to i2clipart.com? =
No.


== Screenshots ==

1. screenshot-1.png: how the plugin looks like when opens up. You can search for an image by typing the proper keywords or provide its dominant color through the color palette.
2. screenshot-2.png: image dominant color search results.
3. screenshot-3.png: Every image comes in four different sizes (64px, 128px, 256px, 512px).
4. screenshot-4.png: how the image looks like in the TinyMCE editor.
5. screenshot-5.png: image keyword search results.
6. screenshot-6.png: Every image is available in several colors.
7. screenshot-7.png: how image variations look like in the TinyMCE editor.